...
public class ProjectNormalizationNameRewriter extends FailsafeNameRewriter {
	...
	public IMethodName rewrite(IMethodName n) {
		ITypeName retType = n.getReturnType();
		ITypeName normRetType = rewrite(retType);

		String normMethod = n.getIdentifier().replace(retType.getIdentifier(), normRetType.getIdentifier());
		return Names.newMethod(normMethod);
	}
