...
public class EpisodeGraphGeneratorTest {
	...
	public void structureIsCreated() throws Exception {
		...
		when(mappingParser.parse()).thenReturn(createMapping(eventMethodDeclGeneralAPI("M1"), eventInvocationSpecificAPI("M2"), 
				eventMethodDeclSpecificAPI("M3"), eventInvocationGeneralAPI("M4"), eventMethodDeclGeneralAPI("M5")));
		
		sut.generateGraphs(frequencyThreshold, bidirectionalThreshold);
		
		File folderStructure = new File(tmpFolderName + "/graphs/" + "/configurationF" + frequencyThreshold + "B"+ bidirectionalThreshold + "/");
	...
	public void generalAPIPatternsStored() throws Exception {
		when(mappingParser.parse()).thenReturn(createMapping(eventMethodDeclGeneralAPI("M1"), eventInvocationGeneralAPI("M2"), eventMethodDeclGeneralAPI("M3"), 
				eventInvocationGeneralAPI("M4"), eventMethodDeclGeneralAPI("M5")));
		
		sut.generateGraphs(frequencyThreshold, bidirectionalThreshold);
		
		File folder = new File(tmpFolderName + "/graphs/" + "/configurationF" + frequencyThreshold + "B"+ bidirectionalThreshold + "/" + "/generalAPI/");
	...
	public void specificAPIPatternsStored() throws Exception {
		when(mappingParser.parse()).thenReturn(createMapping(eventMethodDeclSpecificAPI("M1"), eventInvocationSpecificAPI("M2"), eventMethodDeclSpecificAPI("M3"), 
				eventInvocationSpecificAPI("M4"), eventMethodDeclSpecificAPI("M5")));
		
		sut.generateGraphs(frequencyThreshold, bidirectionalThreshold);
		
		File folder = new File(tmpFolderName + "/graphs/" + "/configurationF" + frequencyThreshold + "B"+ bidirectionalThreshold + "/" + "/specificAPI/");
	...
	private Map<Integer, List<Episode>> createEpisodes() {
		...
		someEpisodes.put(3, newArrayList(newEpisode(1, 3, "1", "3", "5", "3>5"), newEpisode(3, 3, "2", "3", "4", "3>4")));
