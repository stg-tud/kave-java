...
public class MicroCommitIoExtension {
	...
	public Set<List<MicroCommit>> readZipAndSortByLocation(String zip, ICoReTypeName type) {
		Map<ICoReMethodName, List<MicroCommit>> locations = Maps.newHashMap();
		for (MicroCommit mc : contents.get(zip)) {
			if (type.equals(mc.getType())) {
				ICoReMethodName ctx = mc.getMethodContext();
