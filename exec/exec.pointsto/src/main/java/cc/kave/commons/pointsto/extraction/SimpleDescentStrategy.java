...
/**
 * A {@link DescentStrategy} that descends only into non-entry point methods that are not yet contained in the current {@link Callpath}.
 */
public class SimpleDescentStrategy implements DescentStrategy {
	...
	public boolean descent(IMethodDeclaration methodDecl, Callpath currentCallpath) {
		return !methodDecl.isEntryPoint() && !currentCallpath.contains(methodDecl.getName());
	}
