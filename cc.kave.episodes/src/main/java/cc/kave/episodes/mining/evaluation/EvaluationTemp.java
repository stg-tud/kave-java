...
public class EvaluationTemp {
	...
	@Inject
	public EvaluationTemp(ValidationContextsParser parser, EventMappingParser mappingParser, 
			...
			for (Episode e : categoryEntry.getValue()) {
				...
				for (Map.Entry<Double, Set<Episode>> queryEntry : queries.entrySet()) {
					...
					for (Episode query : queryEntry.getValue()) {
						...
						if (proposals.size() > 0) {
							hasProposals = true;
							List<Double> propEvals = new LinkedList<Double>();
							for (Tuple<Episode, Double> tuple : proposals) {
								...
								double getBestEval = getMax(propEvals, eval);
								avgTargetProposal.get(queryEntry.getKey()).get(propCount).addValue(getBestEval);
								propEvals.add(eval);
								propCount++;
							}
						}
...
//			
//			for (int ind = 0; ind < PROPOSALS; ind++) {
//				double qp = categoryAverager.get(p).get(ind).getFirst().average();
//				double tp = categoryAverager.get(p).get(ind).getSecond().average();
